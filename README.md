...
## Hoster-specific configuration quirks

- beget.com

  | file | option | value | Note |
  | ---- | ------ | ----- | ----------- |
  | boot.nix | `networkMode` | `"cloud-init"` | Beget only tested with cloud-init enabled |

- cloud.yandex.ru

  | file | option | value | Note |
  | ---- | ------ | ----- | ----------- |
  | boot.nix | `networkMode` | `"dhcp"` | Yandex tested without cloud-init, so it is better to disable it |
